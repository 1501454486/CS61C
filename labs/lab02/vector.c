...
vector_t *bad_vector_new() {
    ...
    // BAD2: If allocation failed, you should free what you have malloced.

    retval->data[0] = 0;
    // BAD3: If allocation failed, data[0] cannot be visited.
    return retval;
}
...
void vector_set(vector_t *v, size_t loc, int value) {
    ...
    if( loc >= v->size ) {
        ...
        size_t newSize = (loc+1 > 2 * v->size) ? (loc+1) : 2 * v->size;
